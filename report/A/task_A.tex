...
\midrule
Ni  & \SI{420}{eV} & \(8\times8\times8\)   & \SI{0.88}{meV/atom} \\
NiO & \SI{675}{eV} & \(6\times6\times6\)   & \SI{0.94}{meV/atom} \\
\bottomrule
\end{tabular}
\end{table}
...
\(16\times16\times16\) for NiO.  The reason for keeping this numerical margin
is discussed below.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/task_A_ENCUT_convergence.pdf}
\caption{\texttt{ENCUT} convergence for Ni and NiO.  The plotted values are
final total energies per atom extracted from the raw VASP \texttt{OUTCAR}
files.}
\label{fig:task-a-encut}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/task_A_KPOINTS_convergence.pdf}
\caption{k-point convergence for Ni and NiO.  The x-axis uses the number of
irreducible k-points written by VASP in \texttt{IBZKPT}, after symmetry
reduction of the requested mesh.  The non-monotonic but small oscillations in
the Ni curve are consistent with metallic Brillouin-zone sampling.}
\label{fig:task-a-kpoints}
\end{figure}
...
The higher converged cutoff for NiO than for Ni is physically reasonable.  In a
PAW plane-wave calculation the required cutoff is controlled by the hardest
species and by how rapidly the valence wave functions vary in the core and
bonding regions.  The Ni calculation contains only metallic Ni--Ni bonding and
uses a single Ni PAW dataset.  NiO contains oxygen, whose compact \(2p\) valence
states and ionic Ni--O bonding usually require a larger plane-wave basis to
represent the charge density and forces at the same accuracy.  In addition, the
DFT+\(U\) treatment of NiO makes the occupied Ni \(3d\) states more localized,
which can also make the total energy more sensitive to basis-set completeness.
Therefore the fact that Ni is converged already near \(\texttt{ENCUT}=420\) eV,
while NiO needs about \(\texttt{ENCUT}=675\) eV for the same
\(1~\mathrm{meV/atom}\) tolerance, is not anomalous; it reflects the harder
chemical environment and the presence of oxygen in the oxide.

The final production cutoffs are intentionally not just the smallest acceptable
values from the table.  Choosing \(\texttt{ENCUT}=520\) eV for Ni and
\(\texttt{ENCUT}=700\) eV for NiO leaves a margin against small changes caused
by relaxation, magnetism, DFT+\(U\), stress calculations, and phonon or elastic
finite differences.  This is important because later tasks compare energy
differences, forces, stresses, and derived response properties, all of which
are more vulnerable to numerical noise than a single static total energy.
