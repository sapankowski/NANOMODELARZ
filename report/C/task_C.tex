...
\label{tab:task-c-summary}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llllrl}
\toprule
System & Case & Character & Gap type & Gap (eV) & Dominant orbitals near \(E_F\) \\
\midrule
Ni  & PBE FM          & metallic   & metallic & 0.000 & Ni \(d\), Ni \(p\), Ni \(s\) \\
NiO & PBE AFM-II      & insulating & indirect & 0.597 & Ni \(d\), O \(p\), Ni \(p\) \\
NiO & DFT+\(U\) AFM-II & insulating & indirect & 3.403 & O \(p\), Ni \(d\), Ni \(p\) \\
\bottomrule
\end{tabular}%
}
\end{table}

The band structure and DOS show bands crossing the
...
Experimentally, elemental Ni is a ferromagnetic metal rather than a
semiconductor, so the zero-gap result and finite Fermi-level DOS are the
correct qualitative real-material behavior \cite{kittel,crangle-goodman}.  The
same metallic band-crossing picture is also seen in the standard VASP fcc Ni
example, where spin-polarized Ni bands cross the Fermi energy
\cite{vasp-fcc-ni}.
...
The band-edge positions confirm the gap assignment.  In the DFT+\(U\) NiO band
structure of Fig.~\ref{fig:task-c-nio-dftu-bands}, the valence-band maximum is
at \(Z\), with \(E_{\mathrm{VBM}}-E_F=-0.006~\mathrm{eV}\), and the
conduction-band minimum is at \(\Gamma\), with
\(E_{\mathrm{CBM}}-E_F=3.397~\mathrm{eV}\).  The resulting indirect
\(Z\rightarrow\Gamma\) gap is therefore \(3.403~\mathrm{eV}\), the same value
as the DOS-mesh gap in the last row of Table~\ref{tab:task-c-summary}.  This
agreement shows that the plotted band structure contains the same VBM and CBM
identified from the denser DOS calculation.  For comparison, the dense-mesh PBE
NiO extrema are away from the manually plotted high-symmetry line, which is why
the DOS-mesh value in Table~\ref{tab:task-c-summary} is used for the final PBE
gap.
