...
This is the standard VASP workflow for band structures and DOS: first obtain a
...
structure examples as a reference workflow \cite{vasp-fcc-ni}.  The current
...
while keeping the calculation short enough for the project workflow.  A more
strict production workflow would generate the path directly from the relaxed
...
\end{minipage}
\caption{Band structures for Ni PBE FM, NiO PBE AFM-II, and NiO DFT+\(U\)
AFM-II.  Energies are shifted so that \(E_F = 0\).}
\label{fig:task-c-bands}
\end{figure}
...
\end{minipage}
\caption{Total and spin-resolved DOS.  Ni has finite DOS at the Fermi level,
whereas both NiO calculations have a gap.}
\label{fig:task-c-dos}
\end{figure}
...
\end{minipage}
\caption{Projected density of states.  The Ni metal is dominated by Ni
\(d\)-derived states near \(E_F\).  In NiO, O \(p\) and Ni \(d\) states form the
valence and conduction edges, with DFT+\(U\) increasing their separation.}
\label{fig:task-c-pdos}
\end{figure}
...
\end{minipage}
\caption{Projected band structures.  Marker sizes are proportional to selected
\texttt{PROCAR} orbital weights: Ni \(d\) and Ni \(p\) for Ni, and Ni \(d\) and
O \(p\) for NiO.}
\label{fig:task-c-projected-bands}
\end{figure}
